#Haiku Generator Final Verison

import tkinter, random, cmudict, nltk.corpus, inflect, mlconjug3
...
def read(file):#function to read a file
  ...
  try:
    ...
    infoLabel2['text'] = "Error: File "+file+" not found."
...
def hardCodedCheck(word):
 ...
 #function that performs checks and conjugates verb to singular
def conjugateVerb(verb):
  verb = verb.lower() #sanitsation
  
  
  if verb in verbRoots: #checks if verb is already in expected form
    return conjugator.conjugate(verb).conjug_info['indicative']['indicative present']['3s']
  
  elif verb[-2:] in ['ch','ss','sh']:
...
def inputValues():
    ...
    enteredWords = [noun1Entry.get(),noun2Entry.get(),verb1Entry.get(),verb2Entry.get(),adjEntry.get()]
    #print (enteredWords)
...
##                infoLabel2['text'] = "You have exceeded the limit of syllables. Your last input(s) will not be used.")
##                
...
##        else:
##            infoLabel2['text'] = "Words must contain only letters."
...
def makeRandomHaiku():#function that returns are random haiku
  #listOfWords = updateWords()
  #nouns = listOfWords[0]
  #adjectives = listOfWords[1]
...
verbRoots = read('verbRoots.txt')[0]
...
#=========================END OF CODE FROM TEXT-BASED HAIKU=================================


def startHaiku():
    #code from non-gui haiku maker will be used here
    results = inputValues()
    words = results[0]
    syllables = results[1]

    if all(elem == -10 for elem in syllables):
      
      randomHaiku()
    #new code:
    #haiku = makeHaiku(['cow','sea','examines','sways','magnificent'],[1,1,3,1,4])
      
    else:
      infoLabel2['text'] = "Here is your haiku:"
      haiku = makeHaiku(words,syllables)
      haikuLabel['text'] = haiku
      haikuLabel.config(font=("Times", 25))
...
def errorMessage(file): #used to display the error message inside a label if a file isnt found
  infoLabel2['text'] = "Error: File "+file+" not found!"

  
def randomHaiku():
  infoLabel2['text'] = "Here is your haiku:"
  haiku = makeRandomHaiku()
  haikuLabel['text'] = haiku
  haikuLabel.config(font=("Times", 25))

    
def saveHaiku(): #saves the generated haiku when the button for saving is clicked
    #code from non-gui haiku maker will be used here
    haiku = haikuLabel['text']

    if haiku == "*Your haiku will appear here*":
      infoLabel2['text'] = "You need to generate a haiku first!"
      
    else:
      savedHaikus = readHaiku()
      if savedHaikus != "Error":
        file = "haikus.txt"

        if haiku not in savedHaikus:
          try:#try used to display an error message if file isnt found.
        
            myfile = open(file,'a')#file opened
            myfile.write(haiku+'\n')#haiku is appended and a new line is added to declare a new haiku
            myfile.close()
            infoLabel2['text'] = "Haiku saved!"
          except:
            errorMessage(file)

        else:
          infoLabel2['text'] = "This haiku is already saved!"
      else:
        errorMessage(file)
...
def outputHaiku():
  ...
  if savedHaikus == "Error":
    ...
    haikuLabel.config(font=("Times", 25))
    haikuList = savedHaikus.split('\n\n')
    haikuList = haikuList[:-1]#last element which is '' is removed
    #print(haikuList)
    infoLabel2['text'] = "Here is a haiku you have saved before:"
    haikuLabel['text'] = random.choice(haikuList)


#function that updates the theme of the haiku
#it is either changed to make nature adjectives and verbs
#or adjectives and verbs related to abstract values
    
def updateTheme():
  global adjectives, nouns #the global values will change

  #checks if the first box is ticked
  if v.get() == 1:
    
    adjectives = natureAdj
    nouns = natureNoun
    infoLabel2['text'] = "Theme changed to nature."

  #checks if the second box is ticked
  elif v.get()==2:
    adjectives = abstractAdjectives
    nouns = abstractNouns
    infoLabel2['text'] = "Theme changed to 'abstractness'."


window = tkinter.Tk()
window.title("Haiku Generator")
...
infoLabel = tkinter.Label(master = firstFrame, text = "Enter words:",bg="gainsboro")

#frames for each part of gui
noun1Frame = tkinter.Frame(master=firstFrame,bg="gainsboro",pady=10)
noun2Frame = tkinter.Frame(master=firstFrame,bg="gainsboro",pady=10)

verb1Frame = tkinter.Frame(master=firstFrame,bg="gainsboro",pady=10)
verb2Frame = tkinter.Frame(master=firstFrame,bg="gainsboro",pady=10)

adjFrame = tkinter.Frame(master=firstFrame,bg="gainsboro",pady=10)
choiceFrame = tkinter.Frame(master=firstFrame,bg="gainsboro",pady=10)

#entry and label for each value
noun1Label = tkinter.Label(master = noun1Frame, text = "First Noun:",bg="gainsboro")
noun1Entry = tkinter.Entry(master = noun1Frame, width = 20,bg="light yellow")

noun2Label = tkinter.Label(master = noun2Frame, text = "Second Noun:",bg="gainsboro")
noun2Entry = tkinter.Entry(master = noun2Frame, width = 20,bg="light yellow")

verb1Label = tkinter.Label(master = verb1Frame, text = "First Verb:",bg="gainsboro")
verb1Entry = tkinter.Entry(master = verb1Frame, width = 20,bg="light yellow")

verb2Label = tkinter.Label(master = verb2Frame, text = "Second Verb:",bg="gainsboro")
verb2Entry = tkinter.Entry(master = verb2Frame, width = 20,bg="light yellow")

adjLabel = tkinter.Label(master = adjFrame, text = "Adjective:",bg="gainsboro")
adjEntry = tkinter.Entry(master = adjFrame, width = 20,bg="light yellow")

#radio buttons:
v = tkinter.IntVar()
v.set(1)

choiceLabel = tkinter.Label(master = choiceFrame, text = "Choose Theme:",bg="gainsboro")
natureChoice = tkinter.Radiobutton(master = choiceFrame,command = updateTheme,bg='gainsboro',text = "Nature",variable=v,value=1,width = 20)
abstractChoice = tkinter.Radiobutton(master = choiceFrame,bg='gainsboro',command = updateTheme, text = "'Abstractness'",variable=v,value=2,width = 20)

#command = showChoice (write this in RadioButton)
#def showChoice():
#print(v.get())

#every widget and frames are packed into the window
infoLabel.pack()
...
#END OF LEFT HAND SIDE OF GUI


#RIGHT HAND SIDE OF GUI

#info label 2 is the label at the top of the right screen, displaying information of what user did
infoLabel2 = tkinter.Label(secondFrame,bg='gainsboro',text='Welcome to the Haiku Generator!',height=2,fg='red',font=("Calibri",20))
#infoLabel2.grid(row=0,column=0)
infoLabel2.pack()

#frame that will contain choice buttons
buttonFrame = tkinter.Frame(master=secondFrame,bg="gainsboro",pady=15)

startButton = tkinter.Button(master=buttonFrame,padx=20,pady=10,highlightbackground='lightyellow', text = "Make Haiku!", command = startHaiku, height = 2,width = 15)
startButton.grid(row = 0,column = 0,padx=40)

randomButton = tkinter.Button(master=buttonFrame,padx=20,pady=10,highlightbackground='lightyellow', text = "Make a Random Haiku!", command = randomHaiku, height = 2,width = 15)
randomButton.grid(row = 0,column = 1,padx=40)

saveButton = tkinter.Button(master=buttonFrame,padx=20,pady=10,highlightbackground='lightyellow', text = "Save Haiku", command = saveHaiku, height = 2,width = 15)
saveButton.grid(row = 0,column = 2,padx=40)

outputButton = tkinter.Button(master=buttonFrame,padx=20,pady=10, highlightbackground='lightyellow', text = "Output A Saved Haiku", command = outputHaiku, height = 2, width =15)
outputButton.grid(row = 0,column = 3,padx=40)

buttonFrame.pack()

#haiku label is where the haiku will displayed, under the buttons
haikuLabel = tkinter.Label(secondFrame,text='*Your haiku will appear here*',bg='gainsboro',fg='blue',height=10)
haikuLabel.pack()
