#version implementing saving and loading board
...
def startGame(): #function called when the startgame button is pressed 
    global gridFrame,currentPlayer,winCon,end,totalCounters,players,cornerCount,colors,continueRound,playerWins #global variables used due to event driven nature of tkinter
    global currentRound,leaderboardLabel,leaderFrame,playerBombs,currentBomb,isAlliance,allianceColors, alliances
    ...
    alliances = getAlliances(players)
    if alliances == None or players == 1: #if there is 1 player or invalid alliances is entered, no alliances in game
        isAlliance = False

    else: #else there will be alliances
        isAlliance = True
        allianceColors = [] #the colors of each alliance is formed
        for i in range (0,len(alliances)): #loops through the whole loop
            allianceColors.append([])
            for j in range (0,len(alliances[i])):
                allianceColors[i].append(colors[alliances[i][j]]) #the color is appended to the list 
...
def placeCounter(column): #function used to place counter given a column
    ...
    global currentBomb,playerBombs, isAlliance, alliances
    column = column -1
            ...
            #loops through the bottom of the column to the top
                ...
                if board[column][i]['bg'] == 'skyblue':#if it is empty(blue)
                    ...
                    if largest>=winCon: # if this exceeds the winning condition
                        ...
                        if not isAlliance:
                            playerWins[winner] +=1
                        else: #if there are alliances
                            winningAlliance = alliances[listIndex(winner,alliances)] #the winner is found
                            for i in winningAlliance:#for every player in this alliance
                                playerWins[i] += 1 #increase their win
...
def listIndex(item, alliances): #given a list index and the 2d list of alliances
    for i, sublist in enumerate(alliances): #for every element
        if item in sublist: #if i is in the sublist
...
def longestLine(index1,index2,colors):  #function used to find the longest line of a color
    ...
    if isAlliance: #if there is an alliance
        colors = allianceColors[listIndex(colors[0],allianceColors)] #the colors are stored for when calculating lines
...
def getAlliances(players): #function used to get alliance value
    info = alliancesEntry.get().strip() #the data is read from the entry and stripped
    
    if not any(char.isdigit() for char in info): #if there not an integer in the input
        allianceMessage() #display invalid message
        return None
    
    info = [x.split() for x in info.split(',')] #split every member in each alliance -> 2d list formed

    inAlliance = [False for i in range (players)] #none of the players are in an alliance at the start
    
    for i in range (0,len(info)): #for every alliance
        for j in range (0,len(info[0])): #for every player in an alliance
            try:
                info[i][j] = int(info[i][j]) #the data is converted to an integer
            except:
                allianceMessage()
                return None #if gives an error, return none

            if not (0 < info[i][j] <= players): #if this player is not between the valid range
                allianceMessage()
                return None #return message and none
                
            else:
                info[i][j] -= 1 #the number is reduced by 1 to make an index
                
                if inAlliance[info[i][j]] == True: #if this player is already in an alliance
                    allianceMessage() #return invalid
                    return None
                else: 
                    inAlliance[info[i][j]] = True #this player is set to true

    

    alliances = info 
    for i in range (0,players):
        if inAlliance[i] == False:
            alliances.append([i])

    isAlliance = True
    return alliances
        
def allianceMessage(): #used to display an invalid message
    alliancesEntry.config(fg='red')
    alliancesEntry.delete(0,tkinter.END) #the prewritten text in the entry is deleted
    alliancesEntry.insert(0,'Invalid: not using alliances') #the entry is replaced with text to show that no alliances are taking place

def saveBoard(): #function to save the board state
    if end: #cant save a game if it has ended
        mainInfoLabel['text'] = "You can't save a finished game!"
        return
    myfile = open('boards.txt','a') #file is opened to append to

    line1 = str(players)+ ' '+ str(currentPlayer) + ' ' + str(totalCounters) #the first line is the 3 main info about the game
    x = len(board[0]) #the x value and y values are stored
    y = len(board)
    line2 = str(x) + ' '+ str(y) #line 2is the row and column values
    cells = {} #cells that have counters will be stored here

    for i in range (0,len(board)): #for every cell
        for j in range (0,len(board[i])): 
            if board[i][j]['bg'] != 'skyblue': #if background is not empty
                color = board[i][j]['bg'] #store the color
                cells[color] = cells.get(color,[]) + [(i,j)] #in key color, store the coordinate
                
    myfile.write(line1 + '\n' + line2 + '\n' + str(cells) + '\n\n') #the lines are stored in the line
    myfile.close()#file is closed
    mainInfoLabel['text'] = "Board Saved!" #output to user
    
def loadBoard():
    global players,currentPlayer,totalCounters,end,playerWins,currentRound
    try:
        myfile = open('boards.txt') #making sure the file is in the same directory
    except:
        mainInfoLabel['text'] = "Error: File not found." #error message shown and function terminates
        return

    try: #if there is something wrong in the file, this error is handled
        data = myfile.read().split('\n\n')[-2] #the penultimate data is taken (last line is just empty space)
        myfile.close()

        data = data.split('\n') #split into each lines
        
        gameInfo = [int(x) for x in data[0].split()] #the three game info are stored
        
        players,currentPlayer,totalCounters = gameInfo[0],gameInfo[1],gameInfo[2] #the 3 game info are assigned

        gridFrame.destroy() #the older grid is destroyed
        boardDim = [int(x) for x in data[1].split()] #the dimensions are parsed
        createGrid(boardDim[0],boardDim[1],660,660) #the board is created

        counters = eval(data[2]) #the counters places data is parsed

        for k,v in counters.items(): #for every color and their indexes
            for coord in v: #for each coordinate of a point
                x = coord[0] #the x coordinate is stored
                y = coord[1] #the y coordinate is stored
                board[x][y]['bg'] = k #the cell color is changed to the key color
                
        mainInfoLabel['text'] = 'Board loaded!' #at the end the message is shown
        playerWins = [0]*players #player wins are all 0
        end = False
        currentRound = 0
        displayPlayer() #the next player is displayed
        try:
            nextRoundButton.destroy()
        except:
            pass
    except:
        mainInfoLabel['text'] = 'Error: the file contents are altered.'
...
alliancesFrame = tkinter.Frame(master=firstFrame,bg="gainsboro",pady=10) #the frame that contains the grid stuff

alliancesLabel = tkinter.Label(master = alliancesFrame, text = "Enter Alliances:",bg="gainsboro")#grid label
alliancesEntry = tkinter.Entry(master = alliancesFrame, width = 20,bg="light yellow") #grid entry
alliancesEntry.insert(0,'example: 1 3, 2 4') #contains the placerholder to give an idea to user
alliancesEntry.config(fg='grey')

alliancesLabel.pack() #the widgets are packed into the frame
alliancesEntry.pack()
alliancesFrame.pack() #the frame is placed 

alliancesEntry.bind("<FocusIn> ", clearEntry) #when the user presses the entry, it is cleared


loadFrame = tkinter.Frame(master=firstFrame,bg="gainsboro",pady=10) #the frame that contains the grid stuff

#load board
loadButton = tkinter.Button(master=loadFrame,padx=20,pady=10,highlightbackground='lightyellow',
                            text = "Load Saved Board", command = loadBoard, height = 2,width = 15)
loadButton.pack()
loadFrame.pack()
...
#save board
saveButton = tkinter.Button(master=thirdFrame,padx=20,pady=10,highlightbackground='lightyellow',
                            text = "Save Board!", command = saveBoard, height = 2,width = 15)
saveButton.pack()
